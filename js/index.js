...
async function fetchJsonData() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const origin = "http://www.gimhae.go.kr/openapi/tour/restaurant.do";
    const params = "?pageunit=4&page=" + i + 1;
...
function displayData(data) {
  const container = document.getElementsByClassName("card1");
  ...
  if (data) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      writeHTML += '<div class="card1-img">';
      writeHTML += "<img src=" + image + "/>";
      writeHTML += "</div>";
      writeHTML += '<div class="card1-script">dd</div>;';
